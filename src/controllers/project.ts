...
export default {
  getProjects: async function (
    ...
    if(res.locals.user != undefined && res.locals.user.role.name === ROLE.ADMIN){
  ...
  updateProject: async function (
    ...
    const id = params.id;
    body.draft = body.draft? JSON.parse( body.draft ): false;
    const record = await updateProject(parseInt(id, 0), { ...body }, res.locals.user);
    res.status(record.status.code).json(record);
  },
  updateProjectFields: async function (
    ...
    const id = params.id;
    body.draft = body.draft? JSON.parse( body.draft ): false;
    const record = await updateProjectFields(parseInt(id, 0), { ...body }, res.locals.user);
    res.status(record.status.code).json(record);
  },
  ...
  approveProject: async function (
    ...
    const params = req.params;
    const id = params.id;
    const record = await approveProject(parseInt(id, 0), { published: body.published }, res.locals.user);
    res.status(record.status.code).json(record);
  },
  ...
  requestApproveProject: async function (
    ...
    const params = req.params;
    const id = params.id;
    const record = await requestApproveProject(parseInt(id, 0), { requestApproved: body.requestApproved },res.locals.user);
    res.status(record.status.code).json(record);
  },
};
