...
export default {
  ...
  createNews: async function (req: Request, res: Response, next: NextFunction) {
    ...
    const newNews = await createNews({ ...body }, user);
  ...
  updateImages: async function (
    ...
    if (req.files) {
      if (req.files.banner && req.files.banner.length > 0) {
        ...
        let filename =
          uuidv4() + path.extname(req.files.banner[0].originalname);
        ...
        req.body.imageBannerPath =
          "upload/" + process.env.FOLDER_NEWS + "/" + filename;
      }
    }
